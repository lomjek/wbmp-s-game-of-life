...
output_type current_save_mode = NONE;
...
bool load_input_image(char *original_frame) {
	...
	raw_data_size = load_wbmp(original_frame, &img_width, &img_height, &current_structure);
...
void set_pixel(uint32_t x, uint32_t y, bool state) {
	...
	uint32_t width_bytes = img_width / 8;
	uint32_t h_byte_offset = x / 8;
	uint64_t byte_index = y * width_bytes + h_byte_offset;

	uint8_t byte = current_structure[byte_index];

	uint8_t bit_offset = (uint8_t)(7 - (x % 8));
	if (state) {
...
unsigned int get_surrounding_live_pixel_count(uint32_t x, uint32_t y) {
...
void step() {
	*last_structure = *current_structure;
	for (int line = 0; line < img_width; line++) {
		for (int column = 0; column < img_height; column++) {
...
bool save_current_frame() {
	...
	size_t buffer_size = strlen(output_path) + used_digits + 13;
	...
	int result = save_wbmp(frame_path, current_structure, img_width, img_height, raw_data_size);
	if (result != 0) {
		printf("\n");
		wait_for_enter();
	}
...
int main(int argc, char *argv[]){
	...
	if (current_save_mode == ANIMATION){
		...
		save_wbmp(output_path, current_structure, img_width, img_height, raw_data_size);
	}
